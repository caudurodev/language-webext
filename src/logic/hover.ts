...
const WordUnderCursor = {
  ...
  getFullWord(event) {
    let { i, begin, end, range, textNode, offset }
      = WordUnderCursor.getRangeNodeOffset(event)
    ...
    return {
      word: { text: word, start: begin, end, range, textNode, offset },
      sentence,
    }
  },
  /// ///////////
...
const getFullSentence = (range, textNode, offset, event) => {
  ...
  let useParent = false
  if (
    $(event.target).is('a,i,b')
    && $(event.target).parent().is('p,h1,h2,h3')
  ) {
    ...
    const { textNode: newTextNode, offset: newOffset }
      = WordUnderCursor.getRangeNodeOffset(event)
  ...
  sentences.forEach((s) => {
    ...
    instance.mark(searchSentence, {
      ...
      exclude: ['style *', 'script *', 'learnsentence'],
